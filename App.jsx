...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .app-shell {
    ...
    min-height: 100vh;
    padding: 0;
    background: radial-gradient(ellipse at 50% 0%, #1a2a0a 0%, #050505 60%);
  }
  ...
  .phone {
    ...
    box-shadow: 0 0 120px rgba(198,241,53,0.08);
  }
  ...
  .ex-focus-header {
    ...
    background: linear-gradient(135deg, #1a2a0a, #141414);
    display: flex; justify-content: space-between; align-items: flex-start;
  }

  .ex-focus-left { }
  .ex-focus-tag { font-size: 9px; text-transform: uppercase; letter-spacing: 2px; color: var(--lime); font-weight: 700; margin-bottom: 2px; }
  .ex-focus-name { font-family: 'Bebas Neue', sans-serif; font-size: 26px; color: var(--white); letter-spacing: 0.5px; line-height: 1; }
  ...
  .set-table-header {
    display: grid; grid-template-columns: 32px 1fr 1fr 44px;
  ...
  .set-table-header span {
    ...
    color: var(--gray); font-weight: 600; text-align: center;
  }
  ...
  .set-row.active-set { background: rgba(198,241,53,0.05); }
  .set-row.done-set { opacity: 0.55; }
  ...
  .set-num.done-set { color: var(--lime); }
  .set-num.active-set { color: var(--white); }
  ...
  .set-stepper:active { background: rgba(255,255,255,0.14); transform: scale(0.88); }

  .set-stepper.disabled { opacity: 0.25; pointer-events: none; }
  ...
  .set-log-btn:active { transform: scale(0.88); background: rgba(198,241,53,0.25); }
  ...
  .upcoming-title { font-family: 'Bebas Neue', sans-serif; font-size: 16px; color: var(--gray); letter-spacing: 1px; }
  .workout-overall-progress { font-family: 'Space Mono', monospace; font-size: 11px; color: var(--lime); }
  ...
  .upcoming-dot.done { background: var(--lime); }
  .upcoming-dot.next { background: var(--lime); box-shadow: 0 0 8px var(--lime); }
  .upcoming-dot.future { background: #333; }

  .upcoming-info { flex: 1; }
  .upcoming-name { font-size: 13px; font-weight: 600; color: var(--white); }
  .upcoming-name.done { color: #444; text-decoration: line-through; }
  .upcoming-meta { font-size: 11px; color: var(--gray); margin-top: 1px; }
...
// which sets are done: { exId: [true, false, ...] }
...
export default function ForgeApp() {
  ...
  // Per-set log: { exId: [{reps, weight}, ...] }
  const [setLog, setSetLog] = useState({});
  // Per-set done: { exId: [bool, ...] }
  ...
  const [workoutPartial, setWorkoutPartial] = useState(false); // was it finished early?
  const [workoutDuration, setWorkoutDuration] = useState(0);
  // App stats
  const [stats, setStats] = useState(INITIAL_STATS);
  const [workoutLog, setWorkoutLog] = useState([]); // [{date, exercises:[{name,muscle,setsLogged,avgWeight}]}]
  ...
  // ── BUILD LOG ENTRY from current workout session ──
  ...
  // Save partial workout and show completion screen
  ...
  // Total sets done across entire workout
  ...
  // Find current active exercise (first with incomplete sets)
...
function CreateWorkoutScreen({ customExercises, onAddCustomExercise, onBack, onSave }) {
  ...
  const fullLibrary = useMemo(() => {
    // Custom exercises go at the front so they're visible first in the unfiltered list
  ...
  const addCustomExercise = () => {
    ...
    // Persist to shared library for future workouts
    const libraryEntry = {
      id: `custom_${Date.now()}`,
      name: trimmedName,
    ...
    // Also add directly to this workout
    setExercises(prev => [...prev, {
      id: Date.now() + Math.random(),
      name: trimmedName,
      muscle: trimmedMuscle,
        ...
        <div style={{ padding: "14px 0 0" }}>
          ...
          {showSearch && (
            <div className="search-results">
              {/* ── CUSTOM EXERCISE ENTRY (always first) ── */}
              {!showCustomForm ? (
                <div
...
function Stepper({ value, onDown, onUp, disabled }) {
  const isText = typeof value === "string";
  return (
    <div className="set-input-wrap">
      <div className={`set-stepper ${disabled || isText ? "disabled" : ""}`} onClick={!disabled && !isText ? onDown : undefined}>−</div>
      <div className={`set-value ${disabled ? "done" : ""}`}>{isText ? value : value}</div>
      <div className={`set-stepper ${disabled || isText ? "disabled" : ""}`} onClick={!disabled && !isText ? onUp : undefined}>+</div>
    </div>
  );
}
...
function WorkoutScreen({
  ...
  // Total sets done for a given exercise
...
function ProgressScreen({ workoutLog }) {
  // ── Derive analytics from workoutLog ──────────────────────────────────────
  ...
  // Only show muscles that appear in logs or have > 0 sets
  ...
  // Upper/lower ratio from this week
  ...
  // Load trend: daily average max weight across this month's sessions
  // Group by day, compute avg max weight per day
  const dayMap = {};
  ...
  // Best improved lifts this month: compare first vs last logged max weight per exercise
  const liftHistory = {};
  thisMonthLog.forEach(w => {
    w.exercises.forEach(e => {
  ...
  // ── Derive workouts count and streak from workoutLog ──────────────────────
  const loggedWorkoutsCount = workoutLog.length;

  // Streak: count consecutive calendar days (backwards from today) with at least one workout
  const loggedDays = new Set(workoutLog.map(w => w.date.slice(0, 10)));
  let streak = 0;
  const cursor = new Date(now);
  // Check today first; if no workout today, start checking from yesterday
