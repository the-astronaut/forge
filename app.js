...
useEffect(() => {
  ...
  const finishFreestyle = useCallback((sessionExercises, duration) => {
    const loggedExercises = sessionExercises
      .filter(ex => ex.sets.length > 0)
      .map(ex => {
        const weights = ex.sets.map(s => typeof s.weight === "number" ? s.weight : null).filter(w => w !== null);
        const avgWeight = weights.length ? weights.reduce((a, b) => a + b, 0) / weights.length : null;
        const maxWeight = weights.length ? Math.max(...weights) : null;
        return { name: ex.name, muscle: ex.muscle || "Other", setsLogged: ex.sets.length, avgWeight, maxWeight };
      });
    ...
    const totalSetsLogged = loggedExercises.reduce((a, e) => a + e.setsLogged, 0);

    setWorkoutLog(prev => [...prev, { date: new Date().toISOString(), workoutName: "Freestyle Session", exercises: loggedExercises }]);
    setStats(s => ({
      ...s,
      streak: s.streak + 1,
      workoutsThisWeek: Math.min(s.workoutsThisWeek + 1, 7),
      totalWorkouts: s.totalWorkouts + 1,
      weeklyVolume: s.weeklyVolume.map((v, i) => i === 6 ? v + Math.min(totalSetsLogged * 2, 30) : v),
    }));
    setTab("home");
  }, []);
  ...
  return (
    <>
      ...
      <div className="app-shell">
        <div className="phone">
          {tab === "home" && <HomeScreen stats={stats} profile={profile} customWorkouts={customWorkouts} onStartWorkout={startWorkout} onNavigate={setTab} onFreestyle={() => setTab("freestyle")} />}
          ...
          {tab !== "workout" && tab !== "create" && tab !== "freestyle" && (
...
function HomeScreen({ stats, profile, customWorkouts, onStartWorkout, onNavigate, onFreestyle }) {
 ...
 <div className="profile-affirmation">"{affirmation}"</div>
        <div
          ...
          style={{
            margin: "14px 24px 0",
            background: "linear-gradient(135deg, rgba(198,241,53,0.12), rgba(198,241,53,0.03))",
            border: "1.5px solid rgba(198,241,53,0.35)",
            borderRadius: 18,
            padding: "14px 16px",
        ...
        >
          <div style={{ width: 46, height: 46, borderRadius: 14, background: "rgba(198,241,53,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>⚡</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 19, color: "var(--white)", letterSpacing: 0.5, lineHeight: 1.1 }}>BUILD ON THE FLY</div>
            <div style={{ fontSize: 11, color: "var(--gray)", marginTop: 2 }}>Gym's packed? Log whatever you can grab.</div>
          </div>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--lime)" strokeWidth="2.5" strokeLinecap="round"><path d="M9 18l6-6-6-6"/></svg>
        </div>
...
// ─── FREESTYLE ("BUILD ON THE FLY") SCREEN ──────────────────────────────────

function FreestyleWorkoutScreen({ customExercises, onAddCustomExercise, onFinish, onCancel }) {
  // sessionExercises: [{ id, name, muscle, isUnilateral, sets: [{reps, weight}] }]
  ...
  const fullLibrary = useMemo(() => {
    const customAsLibrary = customExercises.map(e => ({ ...e, isCustom: true }));
    return [...customAsLibrary, ...EXERCISE_LIBRARY];
  }, [customExercises]);
  ...
  const filteredLibrary = search.trim().length > 0
    ...
    : fullLibrary.slice(0, 8);

  const activeEx = sessionExercises.find(e => e.id === activeExId) || null;
  const totalSetsLogged = sessionExercises.reduce((a, e) => a + e.sets.length, 0);

  const selectExercise = (libEx) => {
    const existing = sessionExercises.find(e => e.name === libEx.name);
    if (existing) {
      setActiveExId(existing.id);
      const lastSet = existing.sets[existing.sets.length - 1];
      setDraftReps(lastSet ? lastSet.reps : libEx.defaultReps);
      setDraftWeight(lastSet && typeof lastSet.weight === "number" ? lastSet.weight : (typeof libEx.defaultWeight === "number" ? libEx.defaultWeight : 20));
    } else {
      const newEx = {
        id: Date.now() + Math.random(),
        name: libEx.name,
        muscle: libEx.muscle,
        isUnilateral: libEx.isUnilateral || false,
        weightType: libEx.defaultWeight === "BW" || libEx.defaultWeight === "—" ? libEx.defaultWeight : "kg",
        sets: [],
      };
      setSessionExercises(prev => [...prev, newEx]);
      setActiveExId(newEx.id);
      setDraftReps(libEx.defaultReps || 10);
      setDraftWeight(typeof libEx.defaultWeight === "number" ? libEx.defaultWeight : 20);
    }
    setSearch("");
    setShowSearch(false);
  };
  ...
  const logDraftSet = () => {
    ...
    const weightVal = activeEx?.weightType === "BW" ? "BW" : activeEx?.weightType === "—" ? "—" : draftWeight;
    setSessionExercises(prev => prev.map(ex =>
      ex.id === activeExId ? { ...ex, sets: [...ex.sets, { reps: draftReps, weight: weightVal }] } : ex
    ));
  };

  const adjustDraft = (field, delta) => {
    if (field === "reps") setDraftReps(r => Math.max(1, r + delta));
    if (field === "weight") setDraftWeight(w => Math.max(0, w + delta));
  };

  const switchToExercise = (ex) => {
    setActiveExId(ex.id);
    const lastSet = ex.sets[ex.sets.length - 1];
    if (lastSet) {
      setDraftReps(lastSet.reps);
      if (typeof lastSet.weight === "number") setDraftWeight(lastSet.weight);
    }
  };

  const removeExercise = (id) => {
    setSessionExercises(prev => prev.filter(e => e.id !== id));
    if (activeExId === id) setActiveExId(null);
  };

  const handleFinish = () => onFinish(sessionExercises, duration);
  ...
  return (
    <div className="screen" style={{ position: "relative" }}>
      <div className="workout-topbar">
        ...
        <div className="live-badge"><div className="live-dot" />FREESTYLE</div>
        <div className="elapsed-badge">{formatDuration(duration)}</div>
      </div>

      <div style={{ padding: "6px 20px 10px" }}>
        <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 26, color: "var(--white)", letterSpacing: 1, lineHeight: 1 }}>BUILD ON THE FLY</div>
        <div style={{ fontSize: 12, color: "var(--gray)", marginTop: 3 }}>{totalSetsLogged} set{totalSetsLogged !== 1 ? "s" : ""} logged · no rest timer, go at your pace</div>
      </div>
      ...
      <div className="scroll-area">
        ...
        {showSearch && (
          <div className="search-results" style={{ margin: "6px 20px 0" }}>
            {!showCustomForm ? (
              <div className="custom-ex-trigger" onClick={() => { setShowCustomForm(true); setTimeout(() => customNameRef.current?.focus(), 50); }}>
                <div className="custom-ex-icon">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--lime)" strokeWidth="3" strokeLinecap="round"><path d="M12 5v14M5 12h14"/></svg>
                </div>
                <div>
                  <div className="custom-ex-label">Create custom exercise</div>
                  <div className="custom-ex-sub">Not in the list? Add it in seconds</div>
                </div>
              </div>
            ) : (
              <div className="custom-ex-form">
                ...
                <div className="muscle-chips">
                  {["Chest","Back","Shoulders","Biceps","Triceps","Legs","Core","Cardio","Other"].map(m => (
                    <div key={m} className={`muscle-chip pressable ${customExMuscle === m ? "selected" : ""}`} onClick={() => setCustomExMuscle(prev => prev === m ? "" : m)}>{m}</div>
                  ))}
                </div>
                <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
                  {[{ val: "kg", label: "🏋️ Weighted" }, { val: "BW", label: "🤸 Bodyweight" }].map(opt => (
                    <div key={opt.val} className={`muscle-chip pressable ${customExWeightType === opt.val ? "selected" : ""}`} style={{ flex: 1, textAlign: "center", padding: "7px 0" }} onClick={() => setCustomExWeightType(opt.val)}>{opt.label}</div>
                  ))}
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="pressable" style={{ height: 36, borderRadius: 10, background: "var(--card)", border: "1px solid var(--border)", color: "var(--gray)", cursor: "pointer", padding: "0 14px", fontSize: 13, fontWeight: 600 }} onClick={() => { setShowCustomForm(false); setCustomExName(""); setCustomExMuscle(""); }}>Cancel</button>
                  <button className="custom-add-btn pressable" disabled={!customExName.trim()} onClick={addCustomExercise}>ADD & START</button>
                </div>
              </div>
            )}
            {filteredLibrary.length === 0 && search.trim() ? (
              ...
              <div key={ex.id} className="search-result-item" onClick={() => selectExercise(ex)}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <div className="sri-name">{ex.name}</div>
                    {ex.isCustom && <div style={{ fontSize: 9, fontWeight: 700, color: "var(--lime)", background: "rgba(198,241,53,0.1)", border: "1px solid rgba(198,241,53,0.25)", borderRadius: 5, padding: "1px 5px", letterSpacing: 0.5 }}>MY</div>}
                  </div>
                  <div className="sri-muscle">{ex.muscle}</div>
                </div>
                <div className="sri-add">
        ...
        {/* Active exercise quick-logger */}
        {activeEx && (
          <div style={{ margin: "16px 20px 0", background: "var(--card)", border: "1.5px solid rgba(198,241,53,0.3)", borderRadius: 18, padding: "14px 16px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              <div>
                <div style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: 2, color: "var(--lime)", fontWeight: 700, marginBottom: 2 }}>Logging Now</div>
                <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 24, color: "var(--white)", letterSpacing: 0.5, lineHeight: 1 }}>{activeEx.name}</div>
              </div>
              <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 12, color: "var(--gray)" }}>{activeEx.sets.length} set{activeEx.sets.length !== 1 ? "s" : ""}</div>
            </div>

            <div style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 14 }}>
              <div style={{ textAlign: "center" }}>
                <div className="config-label" style={{ marginBottom: 7 }}>REPS</div>
                <div className="config-stepper-row" style={{ gap: 8 }}>
                  <div className="cfg-btn pressable" style={{ width: 30, height: 30, fontSize: 16 }} onClick={() => adjustDraft("reps", -1)}>−</div>
                  <div className="cfg-val" style={{ fontSize: 18, minWidth: 30 }}>{draftReps}</div>
                  <div className="cfg-btn pressable" style={{ width: 30, height: 30, fontSize: 16 }} onClick={() => adjustDraft("reps", 1)}>+</div>
                </div>
              </div>
              {activeEx.weightType !== "BW" && activeEx.weightType !== "—" && (
                <div style={{ textAlign: "center" }}>
                  <div className="config-label" style={{ marginBottom: 7 }}>WEIGHT</div>
                  <div className="config-stepper-row" style={{ gap: 8 }}>
                    <div className="cfg-btn pressable" style={{ width: 30, height: 30, fontSize: 16 }} onClick={() => adjustDraft("weight", -2.5)}>−</div>
                    <div className="cfg-val" style={{ fontSize: 18, minWidth: 50 }}>{draftWeight}kg</div>
                    <div className="cfg-btn pressable" style={{ width: 30, height: 30, fontSize: 16 }} onClick={() => adjustDraft("weight", 2.5)}>+</div>
                  </div>
                </div>
              )}
            ...
            {activeEx.sets.length > 0 && (
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 12 }}>
                {activeEx.sets.map((s, i) => (
                  <div key={i} style={{ background: "rgba(198,241,53,0.08)", border: "1px solid rgba(198,241,53,0.2)", borderRadius: 8, padding: "4px 9px", fontSize: 11, fontFamily: "'Space Mono', monospace", color: "var(--lime)" }}>
                    {s.reps}{typeof s.weight === "number" ? `×${s.weight}kg` : ` ${s.weight}`}
                  </div>
                ))}
              </div>
            )}
        ...
        {sessionExercises.length > 0 && (
          <div style={{ margin: "16px 20px 0" }}>
            <div className="field-label" style={{ marginBottom: 8 }}>This Session</div>
            {sessionExercises.map(ex => (
              <div
                key={ex.id}
                className="pressable"
                onClick={() => switchToExercise(ex)}
                style={{
                  display: "flex", alignItems: "center", justifyContent: "space-between",
                  background: ex.id === activeExId ? "rgba(198,241,53,0.05)" : "var(--card)",
                  border: `1px solid ${ex.id === activeExId ? "rgba(198,241,53,0.25)" : "var(--border)"}`,
                  borderRadius: 12, padding: "10px 12px", marginBottom: 8, cursor: "pointer",
                }}
              >
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "var(--white)" }}>{ex.name}</div>
                  <div style={{ fontSize: 10, color: "var(--gray)", marginTop: 1 }}>{ex.sets.length} set{ex.sets.length !== 1 ? "s" : ""} logged</div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  {ex.sets.length === 0 && (
                    <div className="remove-btn" onClick={(e) => { e.stopPropagation(); removeExercise(ex.id); }}>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--red)" strokeWidth="2.5" strokeLinecap="round"><path d="M18 6L6 18M6 6l12 12"/></svg>
                    </div>
                  )}
                  {ex.id !== activeExId && (
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--gray)" strokeWidth="2.5" strokeLinecap="round"><path d="M9 18l6-6-6-6"/></svg>
                  )}
                </div>
              </div>
            ))}
        ...
        {sessionExercises.length === 0 && (
          <div className="empty-builder" style={{ margin: "16px 20px 0" }}>
            <div className="empty-builder-icon">⚡</div>
            <div className="empty-builder-text">Search above and tap an exercise to start logging</div>
          </div>
        )}
      ...
      <div className="cta-area">
        <div className="cta-row">
          <button
            className="cta-btn pressable"
            style={{
              background: totalSetsLogged > 0 ? "var(--lime)" : "var(--card)",
              border: totalSetsLogged > 0 ? "none" : "1px solid var(--border)",
              color: totalSetsLogged > 0 ? "var(--black)" : "#444",
              cursor: totalSetsLogged > 0 ? "pointer" : "default",
              opacity: totalSetsLogged > 0 ? 1 : 0.5,
            }}
            onClick={totalSetsLogged > 0 ? handleFinish : undefined}
          >
            {totalSetsLogged > 0 ? "🏆 FINISH SESSION" : "LOG A SET TO FINISH"}
          </button>
        </div>
      </div>
      ...
      {showCancelConfirm && (
        <div className="complete-overlay" style={{ gap: 0 }}>
          ...
          <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 32, color: "var(--white)", letterSpacing: 2, textAlign: "center", lineHeight: 1, marginBottom: 8 }}>
            LEAVE FREESTYLE?
          </div>
          {totalSetsLogged > 0 ? (
            <div style={{ fontSize: 13, color: "var(--lime)", textAlign: "center", marginBottom: 28 }}>
              You've logged {totalSetsLogged} set{totalSetsLogged !== 1 ? "s" : ""} — save them instead?
            </div>
          ) : (
            <div style={{ fontSize: 14, color: "var(--gray)", textAlign: "center", marginBottom: 28 }}>Nothing logged yet.</div>
          )}
          <div style={{ display: "flex", flexDirection: "column", gap: 10, width: "100%" }}>
            {totalSetsLogged > 0 && (
              <button className="cta-btn pressable" style={{ background: "rgba(198,241,53,0.12)", border: "1.5px solid rgba(198,241,53,0.35)", color: "var(--lime)" }} onClick={() => { setShowCancelConfirm(false); handleFinish(); }}>
                ✓ SAVE & FINISH
              </button>
            )}
            <button className="cta-btn pressable" style={{ background: "rgba(255,69,69,0.1)", border: "1px solid rgba(255,69,69,0.25)", color: "var(--red)" }} onClick={onCancel}>
              LEAVE WITHOUT SAVING
            </button>
            <button className="cta-btn pressable" style={{ background: "var(--card)", border: "1px solid var(--border)", color: "var(--gray)" }} onClick={() => setShowCancelConfirm(false)}>
              KEEP GOING
            </button>
          </div>
        </div>
