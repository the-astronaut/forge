        ...
        <div className="phone">
          ...
          {tab === "create" && (
            <CreateWorkoutScreen
              ...
              onSave={(w, mode) => {
                if (mode === "overwrite" && editingWorkout) {
                  setCustomWorkouts(prev => prev.map(cw => cw.id === editingWorkout.id ? { ...w, id: editingWorkout.id } : cw));
                } else {
...
function ExploreScreen({ workouts, categories, filterCategory, setFilterCategory, onStartWorkout, onCreateWorkout, onEditWorkout }) {
...
function CreateWorkoutScreen({ customExercises, onAddCustomExercise, onBack, onSave, initialWorkout, isCustomWorkout }) {
  ...
  const commitSave = (mode) => {
    ...
    onSave(buildWorkoutObject(), mode);
  };

  const handleSaveClick = () => {
    if (!canSave) return;
    if (isEditing && isCustomWorkout) {
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body" style={{ paddingTop: 0 }}>
              <div style={{ fontSize: 13, color: "var(--gray)", lineHeight: 1.5, marginBottom: 4 }}>
                Update <span style={{ color: "var(--white)", fontWeight: 700 }}>"{initialWorkout?.name}"</span> directly, or keep the original and save this as a brand new workout.
              </div>
            </div>
